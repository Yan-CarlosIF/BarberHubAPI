...
describe('createAdminService', () => {
  ...
  it('should be able to create an admin user using barber shop slug', async () => {
    const barberShopSlug = barberShopRepositoryInMemory.barberShops[0].slug;
    ...
    const createdUser =
      await userRepositoryInMemory.findByEmail('admin2@example.com');

    expect(createdUser).toBeDefined();
    expect(createdUser?.name).toBe(adminData.name);
    expect(createdUser?.email).toBe('admin2@example.com');
    expect(createdUser?.role).toBe('ADMIN');
  });
